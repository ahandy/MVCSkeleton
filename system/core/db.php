...
abstract class DbBlueprint {
	...
	abstract public function getRow($param, $table, $single = TRUE, $random = false);
...
class Db extends DbBlueprint {
	...
	**/
	public function getRow($param, $table, $single = true, $random = false) {
		$this -> checkTable($table);
		...
		if($single) return $sql_query -> fetch();
		else return $sql_query -> fetchAll();
	}
