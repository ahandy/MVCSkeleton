...
if(!file_exists(CONFIG_PATH . 'settings.php')) {
	...
	echo "The settings.php file was not found in " . CONFIG_PATH;
	exit();
}
...
if(isset($routes -> method_name) && !empty($routes -> method_name) && !isset($dontCallMethod)) {
	if(!isset($routes -> method_parameters) || !is_array($routes -> method_parameters)) {
		$controller_class -> $routes -> method_name;
	}
